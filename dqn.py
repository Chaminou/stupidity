...
class DQN(rl_agent.AbstractAgent):
  ...
  def save(self, checkpoint_dir, player, version):
    for name, saver in self._savers:
      path = saver.save(
          self._session,
          checkpoint_dir + "_" + name + "_" + player + "_" + version)

  def restore(self, checkpoint_dir, player, version):
    for name, saver in self._savers:
      full_checkpoint_dir = checkpoint_dir + "_" + name + "_" + player + "_" + version
      saver.restore(self._session, full_checkpoint_dir)


  def step(self, time_step, is_evaluation=False, add_transition_record=True, q_distrib=False):
    ...
    if (not time_step.last()) and (
      ...
      action, probs = self._epsilon_greedy(info_state, legal_actions, epsilon, q_distrib)
    else:
      action = None
      probs = []
  ...
  def _epsilon_greedy(self, info_state, legal_actions, epsilon, q_distrib=False):
    ...
    if np.random.rand() < epsilon:
      ...
      #print("legal_q_values", legal_q_values)
      #print("legal_actions", legal_actions, end="\n"*2)
      if q_distrib :
        probs = legal_q_values
      else :
        probs[action] = 1.0
    return action, probs
